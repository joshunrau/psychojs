...
export class Mouse extends PsychObject {
  ...
  isPressedIn(...args) {
    ...
    const buttonsFound = isInteger(buttonsMaybe)
      ? buttonsMaybe
      : args.find((o) => hasButtons(o) || isInteger(o));
    // Worst case scenario `wanted` ends up being an empty object
    const { buttons: wanted = buttonsFound || buttonsMaybe } =
      buttonsFound || {};
    ...
    if (typeof shape.contains === "function") {
      ...
      const hasButtonPressed = isInteger(wanted)
        ? pressed[wanted] > 0
        : pressed.some((v) => v > 0);
  ...
  mouseMoved(distance, reset = false) {
    ...
    if (typeof reset === "boolean" && reset == false) {
      if (typeof distance === "undefined") {
        return (
          this._prevPos[0] != this._lastPos[0] ||
          this._prevPos[1] != this._lastPos[1]
        );
      } else {
