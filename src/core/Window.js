...
export class Window extends PsychObject {
  ...
  constructor({
    ...
    document.addEventListener("fullscreenchange", (event) => {
      ...
      console.log(
        "windowAlreadyInFullScreen:",
        self._windowAlreadyInFullScreen,
      );
  ...
  _setupPixi() {
    ...
    this._resizeCallback = (e) => {
      ...
      if (
        hasTouchScreen() &&
        window.screen.height - window.visualViewport.height > keyboardHeight
      ) {
        return;
      }
  ...
  _writeLogOnFlip() {
    ...
    for (const entry of this._msgToBeLogged) {
      this._psychoJS.experimentLogger.log(
        entry.msg,
        entry.level,
        logTime,
        entry.obj,
