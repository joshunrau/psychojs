...
export class ServerManager extends PsychObject {
  ...
  async _downloadResources(resources) {
    const response = {
      context:
        "when downloading resources for experiment: " +
        this._psychoJS.config.experiment.name,
      origin: "ServerManager._downloadResources",
    };

    this._psychoJS.logger.debug(
      "downloading resources for experiment: " +
        this._psychoJS.config.experiment.name,
    );
    ...
    for (const name of resources) {
      ...
      if (typeof extension === "undefined") {
        this.psychoJS.logger.warn(
          `"${name}" does not appear to have an extension, which may negatively impact its loading. We highly recommend you add an extension.`,
        );
      }
      ...
      if (typeof pathStatusData === "undefined") {
        throw Object.assign(response, {
          error: name + " has not been previously registered",
        });
      }
      if (pathStatusData.status !== ServerManager.ResourceStatus.REGISTERED) {
        throw Object.assign(response, {
          error:
            name + " is already downloaded or is currently already downloading",
        });
      }
      ...
      if (["csv", "odp", "xls", "xlsx", "json"].indexOf(extension) > -1) {
        preloadManifest.push(
          /*new createjs.LoadItem().set(*/ {
      ...
      // sound files:
        ...
        if (extension === "wav") {
          this.psychoJS.logger.warn(
            `wav files are not supported by all browsers. We recommend you convert "${name}" to another format, e.g. mp3`,
          );
        }
      }
      ...
      // font files:
        ...
        ["ttf", "otf", "woff", "woff2", "eot"].indexOf(pathExtension) > -1
      ) {
    ...
    for (const name of fontResources) {
      ...
      try {
        const newFont = await new FontFace(
          name,
          `url('${pathStatusData.path}') format('${pathExtension}')`,
        ).load();
    ...
    for (const name of surveyModelResources) {
      ...
      try {
        const getResponse = await this._queryServerAPI(
          "GET",
          `surveys/${pathStatusData.path}/model`,
        );
        ...
        pathStatusData.data = new TextEncoder().encode(
          JSON.stringify(getModelResponse["model"]),
        );
        ...
        throw {
          ...response,
          error: `unable to download resource: ${name}: ${util.toString(error)}`,
        };
      }
    }
  ...
  _listResources() {
    const response = {
      context:
        "when listing the resources for experiment: " +
        this._psychoJS.config.experiment.fullpath,
      origin: "ServerManager._listResourcesSession",
    };
    this._psychoJS.logger.debug(
      `listing the resources for experiment: ${this._psychoJS.config.experiment.fullpath}`,
    );
  ...
  async closeSession(isCompleted = false, sync = false) {
    ...
    this._psychoJS.logger.debug(
      "closing the session for experiment: " +
        this._psychoJS.config.experiment.name,
    );
    ...
    if (sync) {
      const url =
        this._psychoJS.config.pavlovia.URL +
        "/api/v2/experiments/" +
        this._psychoJS.config.gitlab.projectId +
        "/sessions/" +
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (deleteResponse.status !== 200) {
            throw "error" in closeSessionResponse
  ...
  getResource(name, errorIfNotDownloaded = false) {
    ...
    if (
      ...
      throw Object.assign(response, {
        error:
          name +
          " is not available for use (yet), its current status is: " +
  ...
  getResourceStatus(names) {
    ...
    if (!Array.isArray(names)) {
      throw Object.assign(response, {
        error: "names should be either a string or an array of strings",
      });
    }
    ...
    for (const name of names) {
      ...
      if (
        statusOrder.get(Symbol.keyFor(pathStatusData.status)) <
  ...
  async getSurveyExperimentParameters(surveyId, experimentInfo) {
    ...
    this._psychoJS.logger.debug(
      `downloading the experiment parameters of survey: ${surveyId}`,
    );
    this.setStatus(ServerManager.Status.BUSY);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (getResponse.status !== 200) {
          throw "error" in getExperimentParametersResponse
            ? getExperimentParametersResponse.error
        ...
        const experimentParameters =
          getExperimentParametersResponse["experimentParameters"];
  ...
  openSession(params = {}) {
    ...
    this._psychoJS.logger.debug(
      "opening a session for experiment: " +
        this._psychoJS.config.experiment.fullpath,
    );
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (postResponse.status !== 200) {
          throw "error" in openSessionResponse
        ...
        self._psychoJS.config.experiment.saveFormat = Symbol.for(
          experiment.saveFormat,
        );
        self._psychoJS.config.experiment.saveIncompleteResults =
        ...
        resolve({
          ...response,
          status: openSessionResponse.status,
          token: openSessionResponse.token,
        });
      } catch (error) {
        console.error(error);
  ...
  async prepareResources(resources = []) {
    const response = {
      context:
        "when preparing resources for experiment: " +
        this._psychoJS.config.experiment.name,
      origin: "ServerManager.prepareResources",
    };

    this._psychoJS.logger.debug(
      "preparing resources for experiment: " +
        this._psychoJS.config.experiment.name,
    );
    ...
    try {
      ...
      if (resources !== null) {
        ...
        if (
          this._psychoJS.config.environment ===
          ...
          for (const name of serverResponse.resources) {
            if (!this._resources.has(name)) {
              ...
              this._psychoJS.logger.debug(
                `registered resource: name= ${name}, path= ${path}`,
              );
        ...
        // then we register those specific resources, if they have not been registered already
          ...
          if (
            this._psychoJS.config.environment ===
              ExperimentHandler.Environment.LOCAL &&
            allResources
          ) {
          ...
          for (let { download, name, path } of resources) {
            if (!this._resources.has(name)) {
  ...
  setStatus(status) {
    const response = {
      context:
        "when changing the status of the server manager to: " +
  ...
  async uploadAudioVideo({
    ...
    const response = {
      context:
        "when uploading media data for experiment: " +
        this._psychoJS.config.experiment.fullpath,
    ...
    try {
      ...
      this._psychoJS.logger.debug(
        `uploading media data for experiment: ${this._psychoJS.config.experiment.fullpath}`,
      );
      this.setStatus(ServerManager.Status.BUSY);
      ...
      const filename =
        participant + "_" + experimentName + "_" + datetime + "_" + tag;
      ...
      this._psychoJS.logger.debug(
        `post media response: ${JSON.stringify(postMediaResponse)}`,
      );
      ...
      if (waitForCompletion) {
        ...
        while (true) {
          ...
          this._psychoJS.logger.debug(
            `check upload status response: ${JSON.stringify(checkStatusResponse)}`,
          );

          if (
            "status" in checkStatusResponse &&
            checkStatusResponse["status"] === "COMPLETED"
          ) {
            break;
          }
        }
  ...
  uploadData(key, value, sync = false) {
    ...
    const checkThrottling =
      typeof this._psychoJS.config.experiment.resultsUpload
        .lastUploadTimestamp !== "undefined";
    if (
      checkThrottling &&
      now - this._psychoJS.config.experiment.resultsUpload.lastUploadTimestamp <
        this._uploadThrottlePeriod * 60
    ) {
      return Promise.reject({
        ...response,
        error: "upload canceled by throttling",
      });
    }
    ...
    if (sync) {
      ...
      navigator.sendBeacon(
        `${this._psychoJS.config.pavlovia.URL}/api/v2/${path}`,
        formData,
      );
    }
    // asynchronously query the pavlovia server:
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          if (postResponse.status !== 200) {
            throw "error" in uploadDataResponse
              ? uploadDataResponse.error
              : uploadDataResponse;
          }
  ...
  uploadLog(logs, compressed = false) {
    const response = {
      context:
        "when uploading participant's log for experiment: " +
        this._psychoJS.config.experiment.fullpath,
      origin: "ServerManager.uploadLog",
    };
    this._psychoJS.logger.debug(
      "uploading server log for experiment: " +
        this._psychoJS.config.experiment.fullpath,
    );
    ...
    const info = this.psychoJS.experiment.extraInfo;
    const filenameWithoutPath = this.psychoJS.experiment.dataFileName
      .split(/[\\/]/)
      .pop();
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (postResponse.status !== 200) {
          throw "error" in uploadLogsResponse
            ? uploadLogsResponse.error
            : uploadLogsResponse;
        }
  ...
  async uploadSurveyResponse(surveyId, surveyResponse, isComplete) {
    ...
    this._psychoJS.logger.debug(
      `uploading a survey response for experiment: ${this._psychoJS.config.experiment.fullpath} and survey: ${surveyId}`,
    );
    this.setStatus(ServerManager.Status.BUSY);
  ...
  waitForResources(resources = []) {
    ...
    return async () => {
      ...
      if (
        t >= 0.0 &&
        self._waitForDownloadComponent.status === PsychoJS.Status.NOT_STARTED
      ) {
        ...
        if (resources.length === 0) {
          for (const [name, { data, path, status }] of this._resources) {
        ...
        for (let { name, path } of resources) {
          ...
          // the resource has been registered but is not downloaded yet:
            ...
            typeof pathStatusData.status !==
            ServerManager.ResourceStatus.DOWNLOADED
          ) {
