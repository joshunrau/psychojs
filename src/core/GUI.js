...
export class GUI {
  ...
  _onResourceEvents(signal) {
    ...
    // update progress bar:
      ...
      if (signal.message === ServerManager.Event.RESOURCE_DOWNLOADED) {
        this._setProgressMessage(
          `downloaded ${this._progressBarCurrentValue / 2}  / ${this._progressBarMax / 2}`,
        );
      } else {
        this._setProgressMessage(
          `downloading ${this._progressBarCurrentValue / 2}  / ${this._progressBarMax / 2}`,
        );
      }
  ...
  _updateDialog(changeOKButtonFocus = true) {
    const allRequirementsFulfilled =
      this._allResourcesDownloaded &&
      this._setRequiredKeys &&
      this._setRequiredKeys.size >= this._requiredKeys.length;
  ...
  dialog({
    ...
    if (showCancel || showOK) {
      ...
      if (showCancel) {
        markup +=
          "<button id='dialogCancel' class='dialog-button' aria-label='Close dialog'>Cancel</button>";
      }
      if (showOK) {
        markup +=
          "<button id='dialogOK' class='dialog-button' aria-label='Close dialog'>Ok</button>";
      }
  ...
  DlgFromDict({
    ...
    requireParticipantClick = GUI.DEFAULT_SETTINGS.DlgFromDict
    ...
    return () => {
      ...
      if (
        t >= 0.0 &&
        self._dialogComponent.status === PsychoJS.Status.NOT_STARTED
      ) {
        ...
        if (
          ...
          typeof self._psychoJS.config.experiment.license.institutionLogo !==
            "undefined"
        ) {
        ...
        if (typeof logoUrl === "string") {
          markup +=
            '<img id="dialog-logo" class="logo" alt="logo" src="' +
            logoUrl +
            '">';
        }
        ...
        if (self._requireParticipantClick) {
          markup +=
            "<button id='dialogOK' class='dialog-button disabled' aria-label='Start Experiment'>Ok</button>";
        }
        markup += "</div>"; // button-group
