...
export class Logger {
  ...
  _throttle(time) {
    ...
    if (this._serverLogs.length > this._throttling.threshold) {
      const timeAtStartThrottlingWindow =
        this._serverLogs[
          this._serverLogs.length - 1 - this._throttling.threshold
        ].time;
      if (time - timeAtStartThrottlingWindow < this._throttling.window) {
        ...
        if (
          ...
          time - this._throttling.startOfThrottling >
            this._throttling.minimumDuration
        ) {
          this._psychoJS.logger.info(
            `[time= ${time.toFixed(3)}] Log messages are not throttled any longer.`,
          );
    ...
    if (
      ...
      if (typeof pako !== "undefined") {
        try {
          ...
          return await this._psychoJS.serverManager.uploadLog(
            base64DeflatedLogs,
            true,
          );
        } catch (error) {
  ...
  _customConsoleLayout() {
    ...
    const customLayout = new log4javascript.PatternLayout(
      "%p %d{HH:mm:ss.SSS} %f{1} | %m",
    );
    customLayout.setCustomField(
      "location",
      function (layout, loggingReference) {
        // we throw a fake exception to retrieve the stack trace
        try {
          // (0)();
          throw Error("fake exception");
        } catch (e) {
          const stackEntries = e.stack
            .replace(/^.*?\n/, "")
            .replace(/(?:\n@:0)?\s+$/m, "")
            .replace(/^\(/gm, "{anon}(")
            .split("\n");
  ...
  _getValue(level) {
    ...
    return levelAsString in Logger._ServerLevelValue
      ? Logger._ServerLevelValue[levelAsString]
      : 30;
  }
}
