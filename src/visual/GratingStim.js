  ...
  constructor({
    ...
    this._addAttribute(
      ...
      GratingStim.#SHADERS[tex]
        ? GratingStim.#SHADERS[tex].uniforms.uFreq || 1.0
        : 1.0,
    );
    this._addAttribute(
      "phase",
      phase,
      GratingStim.#SHADERS[tex]
        ? GratingStim.#SHADERS[tex].uniforms.uPhase || 0.0
        : 0.0,
    );
    ...
    if (!Array.isArray(this.size) || this.size.length === 0) {
      this.size = util.to_unit(
        GratingStim.#DEFAULT_STIM_SIZE_PX,
        "pix",
        this.win,
        this.units,
      );
  ...
  setTex(tex, log = false) {
    ...
    try {
      ...
      if (typeof tex === "undefined") {
        this.psychoJS.logger.warn(
          "setting the tex of GratingStim: " +
            this._name +
            " with argument: undefined.",
        );
        this.psychoJS.logger.debug(
          "set the tex of GratingStim: " + this._name + " as: undefined",
        );
      } else if (GratingStim.#SHADERS[tex] !== undefined) {
        // tex is a string and it is one of predefined functions available in shaders
        this.psychoJS.logger.debug(
          "the tex is one of predefined functions. Set the tex of GratingStim: " +
            this._name +
            " as: " +
            tex,
        );
