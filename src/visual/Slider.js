...
export class Slider extends util.mix(VisualStim).with(ColorMixin, WindowMixin) {
  ...
  constructor({
    ...
    this._addAttribute("labels", labels, [], onChange(true, true, true));
    this._addAttribute("startValue", startValue, undefined);
    this._addAttribute(
      "granularity",
      granularity,
      0,
    ...
    this._addAttribute("readOnly", readOnly, false);
    this._addAttribute("compact", compact, false, this._onChange(true, true));
    ...
    this._addAttribute("bold", bold, true, this._onChange(true, true));
    this._addAttribute("italic", italic, false, this._onChange(true, true));
    this._addAttribute("flip", flip, false, this._onChange(true, true));
  ...
  _estimateBoundingBox() {
    ...
    for (let l = 0; l < this._labels.length; ++l) {
      const tickPositionIndex = Math.round(
        (l / (this._labels.length - 1)) * (this._ticks.length - 1),
      );
      this._labelPositions_px[l] = this._tickPositions_px[tickPositionIndex];
      const labelBounds = PIXI.TextMetrics.measureText(
        this._labels[l].toString(),
        labelTextStyle,
      );
      ...
      if (this._isHorizontal()) {
        if (this._flip) {
          this._labelPositions_px[l][1] -=
            labelBounds.height + this._tickSize_px[1];
        } else {
        ...
        if (this._style.indexOf(Slider.Style.LABELS_45) === -1) {
          ...
          if (this._compact) {
            this._labelPositions_px[l][0] = Math.min(
              size_px[0] / 2 - labelBounds.width,
              Math.max(-size_px[0] / 2, this._labelPositions_px[l][0]),
            );
          }
          ...
          if (
            prevLabelBounds &&
            this._labelPositions_px[l - 1][0] +
              prevLabelBounds.width +
              tolerance >=
              this._labelPositions_px[l][0]
          ) {
      ...
      // vertical slider:
        ...
        if (this._compact) {
          this._labelPositions_px[l][1] = Math.min(
            size_px[1] / 2 - labelBounds.width,
            Math.max(-size_px[1] / 2, this._labelPositions_px[l][1]),
          );
        }
        if (this._flip) {
          ...
          this._labelPositions_px[l][0] -=
            labelBounds.width + this._tickSize_px[0] * 2;
        } else {
      ...
      limits_px[2] = Math.max(
        limits_px[2],
        this._labelPositions_px[l][0] + labelBounds.width,
      );
      limits_px[3] = Math.max(
        limits_px[3],
        this._labelPositions_px[l][1] + labelBounds.height,
      );
    }
  ...
  _getPosition_px() {
    ...
    if (
      this._compact &&
      (this._style.indexOf(Slider.Style.RADIO) > -1 ||
        this._style.indexOf(Slider.Style.RATING) > -1)
    ) {
      if (this._isHorizontal()) {
        position.y -=
          this._getLengthPix(this._tickSize[1]) * (this._flip ? -1 : 1);
      } else {
        position.x +=
          this._getLengthPix(this._tickSize[0]) * (this._flip ? -1 : 1);
      }
    }
  ...
  _handlePointerDown(e) {
    ...
    if (!this._frozenMarker) {
      ...
      const rating = this._posToRating([
        mouseLocalPos_px.x,
        mouseLocalPos_px.y,
      ]);
  ...
  _setupMarker() {
    ...
    if (this._markerType === Slider.Shape.DISC) {
      ...
      this._marker.lineStyle(
        1,
        this.getContrastedColor(this._markerColor, 0.5).int,
        1,
        0,
      );
      this._marker.beginFill(this._markerColor.int, 1);
  ...
  _setupStyle() {
    const isWhiteOnBlack =
      this._style.indexOf(Slider.Style.WHITE_ON_BLACK) > -1;
    const skin = isWhiteOnBlack
      ? this._skin.WHITE_ON_BLACK
      : this._skin.STANDARD;
    ...
    this._markerSize = !this._skin.MARKER_SIZE
      ? this._tickSize
      : this._skin.MARKER_SIZE;
    ...
    if (this._style.indexOf(Slider.Style.SLIDER) > -1) {
      ...
      if (!this._skin.MARKER_SIZE) {
        this._markerSize = this._isHorizontal()
          ? [
              this._size[0] /
                (this._ticks[this._ticks.length - 1] - this._ticks[0]),
              this._size[1],
            ]
          : [
              this._size[0],
