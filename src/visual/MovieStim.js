...
export class MovieStim extends VisualStim {
  ...
  constructor({
    ...
    this._addAttribute("movie", movie);
    this._addAttribute("volume", volume, 1.0, this._onChange(false, false));
    this._addAttribute("noAudio", noAudio, false, this._onChange(false, false));
    this._addAttribute(
      "autoPlay",
      autoPlay,
      true,
    ...
    this._addAttribute("loop", loop, false, this._onChange(false, false));
    ...
    try {
      ...
      if (typeof movie === "undefined") {
        ...
        this.psychoJS.logger.debug(
          `set the movie of MovieStim: ${this._name} as: undefined`,
        );
      } else {
        ...
        this.psychoJS.logger.debug(
          `set the movie of MovieStim: ${this._name} as: src= ${movie.src}, size= ${movie.videoWidth}x${movie.videoHeight}, duration= ${movie.duration}s`,
        );
