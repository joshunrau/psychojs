...
function handleDropdownRendering(survey, options) {
  ...
  options.htmlElement
    .querySelector(".sd-selectbase")
    .insertAdjacentHTML("beforebegin", selectHTML);

  const selectDOM = options.htmlElement.querySelector("select");
  selectDOM.addEventListener(
    "change",
    handleValueChange.bind(this, survey, options),
  );

  options.question.valueChangedCallback = handleValueChangeForDOM.bind(
    this,
    survey,
    options,
  );
