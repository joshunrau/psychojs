...
class SliderStar {
  ...
  _handleIndividualValueUpdate(v, qIdx) {
    ...
    if (this._question.value[qIdx] !== v) {
      ...
      this._DOM.querySelector(`.slider-star-text-input[name="${qIdx}"]`).value =
        v;
  ...
  _init(question, el) {
    ...
    let starsHTML = new Array(question.starCount)
      .fill(`<div class="star-slider-star-input">★</div>`)
      .join("");
    let html = "";
    let i;
    for (i = 0; i < question.choices.length; i++) {
      html += `<div class="star-slider-container">
    ...
    for (i = 0; i < starsContainers.length; i++) {
      ...
      starsContainers[i].addEventListener(
        "pointerdown",
        this._bindedHandlers._handlePointerDown,
      );
      starsContainers[i].addEventListener(
        "pointermove",
        this._bindedHandlers._handlePointerMove,
      );
    }
