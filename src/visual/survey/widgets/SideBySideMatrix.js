...
class SideBySideMatrix {
  ...
  _generateCheckboxCells(row, col, subColumns, CSS_CLASSES) {
    ...
    for (i = 0; i < subColumns.length; i++) {
      bodyCells += `<td class="${CSS_CLASSES.TABLE_CELL}">
  ...
  _init(question, el) {
    ...
    for (i = 0; i < question.columns.length; i++) {
      if (question.columns[i].cellType !== CELL_TYPES.DROP_DOWN) {
        headerCells += `<th class="${CSS_CLASSES.TABLE_HEADER_CELL}" colspan="${question.columns[i].subColumns.length}">
        ...
        headerCells += `<th class="${CSS_CLASSES.TABLE_HEADER_CELL}">
				${question.columns[i].title}
				</th>`;
    ...
    for (i = 0; i < question.rows.length; i++) {
      ...
      for (j = 0; j < question.columns.length; j++) {
        cellGenerator =
          this[SideBySideMatrix.CELL_GENERATORS[question.columns[j].cellType]];
        if (typeof cellGenerator === "function") {
          ...
          console.log(
            "No cell generator found for cellType",
            question.columns[j].cellType,
          );
        }
      }
    ...
    for (i = 0; i < selectDOMS.length; i++) {
      selectDOMS[i].addEventListener(
        "change",
        this._bindedHandlers._handleSelectChange,
      );
    }
  }
}
...
export default function init(Survey) {
  var widget = {
    ...
    afterRender: function (question, el) {
      new SideBySideMatrix({ el, question });
