...
class MaxDiffMatrix {
  ...
  _handleInput(e) {
    ...
    const colRadioDOMS = this._DOM.querySelectorAll(
      `input[data-column="${col}"]`,
    );
  ...
  _init(question, el) {
    ...
    // Header generation
    let headerCells = `<th class="${CSS_CLASSES.TABLE_HEADER_CELL}">${question.columns[0].text}</th>
		<td></td>
		<td></td>
		<td></td>
    ...
    for (let i = 0; i < question.rows.length; i++) {
      const bodyCells = `<td class="${CSS_CLASSES.TABLE_CELL}">
