 ...
 * @mixes ColorMixin
 */
export class ShapeStim extends util
  .mix(VisualStim)
  .with(ColorMixin, WindowMixin) {
  /**
  ...
  constructor({
    ...
    this._addAttribute("vertices", vertices, [
      [-0.5, 0],
      [0, 0.5],
      [0.5, 0],
    ]);
    ...
    this._addAttribute("lineWidth", lineWidth, 1.5, this._onChange(true, true));
  ...
  _getPixiPolygon() {
    ...
    if (coords_px.length >= 6 && this._closeShape) {
      ...
      if (
        coords_px[0] !== coords_px[n - 2] ||
        coords_px[1] !== coords_px[n - 1]
      ) {
