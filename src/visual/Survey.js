...
export class Survey extends VisualStim {
  ...
  constructor({
    ...
    this._addAttribute("surveyId", surveyId, defaultSurveyId);
  ...
  _applyInQuestionRandomization(
    questionData,
    inQuestionRandomizationSettings,
    surveyData,
  ) {
    ...
    if (questionData.rows !== undefined) {
      ...
      console.log(
        "[Survey runner]: Unknown choicesFieldName for",
        questionData,
      );
    }

    if (inQuestionRandomizationSettings.randomizeAll) {
      questionData[choicesFieldName] = util.shuffle(
        Array.from(questionData[choicesFieldName]),
      );
      ...
      questionData[choicesFieldName] = util.shuffle(
        Array.from(questionData[choicesFieldName]).splice(
          0,
          inQuestionRandomizationSettings.showOnly,
        ),
      );
      ...
      questionData[choicesFieldName] =
        Math.round(Math.random()) === 1
          ? questionData[choicesFieldName].reverse()
          : questionData[choicesFieldName];
      ...
      for (let i = 0; i < questionData[choicesFieldName].length; i++) {
        choicesMap[questionData[choicesFieldName][i][valueFieldName]] =
          questionData[choicesFieldName][i];
      }

      // Creating new array of choices to which we're going to write from randomized/reversed sets.
      questionData[choicesFieldName] = new Array(
        inQuestionRandomizationSettings.layout.length,
      );
      const shuffledSet0 = util.shuffle(
        Array.from(inQuestionRandomizationSettings.set0),
      );
      const shuffledSet1 = util.shuffle(
        Array.from(inQuestionRandomizationSettings.set1),
      );
      ...
      const reversedSet =
        Math.round(Math.random()) === 1
          ? inQuestionRandomizationSettings.reverseOrder.reverse()
      ...
      for (let i = 0; i < inQuestionRandomizationSettings.layout.length; i++) {
        if (inQuestionRandomizationSettings.layout[i] === "set0") {
          questionData[choicesFieldName][i] =
            choicesMap[shuffledSet0[setIndices[0]]];
          setIndices[0]++;
        } else if (inQuestionRandomizationSettings.layout[i] === "set1") {
          questionData[choicesFieldName][i] =
            choicesMap[shuffledSet1[setIndices[1]]];
          setIndices[1]++;
        } else if (
          inQuestionRandomizationSettings.layout[i] === "reverseOrder"
        ) {
          questionData[choicesFieldName][i] =
            choicesMap[reversedSet[setIndices[2]]];
          setIndices[2]++;
        } else {
          questionData[choicesFieldName][i] =
            choicesMap[inQuestionRandomizationSettings.layout[i]];
        }
      }

      if (
        inQuestionRandomizationSettings.layout.length < initialChoices.length
      ) {
  ...
  _onCurrentPageChanging(surveyModel, options) {
    ...
    if (skipLogic !== undefined) {
      this._expressionsRunner.expressionExecutor.setExpression(
        skipLogic.expression,
      );
      ...
      if (result) {
        ...
        if (skipLogic.destination === "ENDOFSURVEY") {
          ...
          this._surveyRunningPromiseResolve(
            Survey.SURVEY_COMPLETION_CODES.SKIP_TO_END_OF_SURVEY,
          );
        } else if (skipLogic.destination === "ENDOFBLOCK") {
          surveyModel.setCompleted();
          this._surveyRunningPromiseResolve(
            Survey.SURVEY_COMPLETION_CODES.SKIP_TO_END_OF_BLOCK,
          );
        } else {
          // skipLogic.destination is a question within the current survey (qualtrics block).
          const targetQuestion = surveyModel.getQuestionByName(
  ...
  _onSurveyJSComplete(node, surveyModel, options) {
    ...
    this._isCompletedAll =
      this._isCompletedAll &&
      nbAnsweredQuestions === surveyVisibleQuestions.length;
...
*/
  ...
  _runQuestionBlock(node, surveyData) {
    ...
    if (!this._surveyJSModel.isInitialized) {
      ...
      this._surveyJSModel.onValueChanged.add(
        this._onQuestionValueChanged.bind(this),
      );
      this._surveyJSModel.onCurrentPageChanging.add(
        this._onCurrentPageChanging.bind(this),
      );
      this._surveyJSModel.onComplete.add((surveyJSModel, options) =>
        this._onSurveyJSComplete(node, surveyJSModel, options),
      );
      this._surveyJSModel.onTextMarkdown.add(this._onTextMarkdown.bind(this));
      this._surveyJSModel.isInitialized = true;
      this._surveyJSModel.onAfterRenderQuestion.add(
        this._handleAfterQuestionRender.bind(this),
      );
    }

    const completeText =
      node.surveyIdx < this._surveyData.surveys.length - 1
  ...
  async _runSurveyFlow(node, surveyData, prevBlockResults = {}) {
    ...
    if (node.type === Survey.SURVEY_FLOW_PLAYBACK_TYPES.CONDITIONAL) {
      ...
      if (this._expressionsRunner.run(dataset) && node.nodes[0] !== undefined) {
        nodeExitCode = await this._runSurveyFlow(
          node.nodes[0],
          surveyData,
          prevBlockResults,
        );
      ...
      for (let j = 0; j < surveyBlockData.length; j++) {
        ...
        if (surveyBlockData[j].type === "Custom") {
          ...
          this._expressionsRunner.expressionExecutor.setExpression(
            surveyBlockData[j].value,
          );
          if (this._expressionsRunner.expressionExecutor.canRun()) {
            this._variables[surveyBlockData[j].key] =
              this._expressionsRunner.run(this._variables);
          } else {
      ...
      console.log(
        "embedded data variables accumulation took",
        performance.now() - t,
      );
    ...
    // QUESTION_BLOCK:
      ...
      const surveyCompletionCode = await this._runQuestionBlock(
        node,
        surveyData,
      );
    ...
    if (
      nodeExitCode === Survey.NODE_EXIT_CODES.NORMAL &&
