...
class Particle {
  ...
  update(dt) {
    ...
    this.x = this.x + this.vx * dt + this.ax * dt2 * 0.5;
    this.y = this.y + this.vy * dt + this.ay * dt2 * 0.5;
...
export class ParticleEmitter {
  ...
  _setupParticle(p) {
    ...
    const spawnOffsetX = Math.random() * spawnAreaWidth - spawnAreaWidth * 0.5;
    const spawnOffsetY =
      Math.random() * spawnAreaHeight - spawnAreaHeight * 0.5;
    ...
    if (Number.isFinite(this._cfg.initialVx)) {
      ...
      this._cfg.initialVx instanceof Array &&
      this._cfg.initialVx.length >= 2
    ) {
      p.vx =
        Math.random() * (this._cfg.initialVx[1] - this._cfg.initialVx[0]) +
        this._cfg.initialVx[0];
    } else {
      p.vx = Math.random() * DEFAULT_PARTICLE_V - DEFAULT_PARTICLE_V * 0.5;
    }

    if (Number.isFinite(this._cfg.initialVy)) {
      p.vy = this._cfg.initialVy;
    } else if (
      this._cfg.initialVy instanceof Array &&
      this._cfg.initialVy.length >= 2
    ) {
      p.vy =
        Math.random() * (this._cfg.initialVy[1] - this._cfg.initialVy[0]) +
        this._cfg.initialVy[0];
    } else {
      p.vy = Math.random() * DEFAULT_PARTICLE_V - DEFAULT_PARTICLE_V * 0.5;
    }
  ...
  update(dt) {
    ...
    for (i = 0; i < this._particlePool.length; i++) {
      ...
      if (
        this._particlePool[i].lifeTime <= 0 &&
        this._particlePool[i].sprite.parent
      ) {
