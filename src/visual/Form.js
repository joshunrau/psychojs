...
export class Form extends util.mix(VisualStim).with(ColorMixin) {
  ...
  constructor({
    ...
    this._addAttribute("items", items, [], onItemChange);
    this._addAttribute("randomize", randomize, false, onItemChange);
  ...
  _importItems() {
    ...
    try {
      ...
      // if items is a string, we treat it as the name of a resource file and import it:
        ...
        this._items = TrialHandler.importConditions(
          this._psychoJS.serverManager,
          this._items,
        );
      }
      // unknown items type:
    ...
    try {
      ...
      for (const item of this._items) {
        ...
        // turn the ticks and tickLabels into arrays, where applicable:
          ...
          item.tickLabels =
            item.tickLabels.length > 0 ? item.tickLabels.split(",") : [];
        }
  ...
  _setupStimuli() {
    if (this._autoLog) {
      this._psychoJS.experimentLogger.exp(
        `Setting layout of Form: ${this.name}`,
      );
    }
    ...
    for (const item of this._items) {
      ...
      if (
        ...
        (item.type === Form.Types.CHOICE &&
          item.layout === Form.Layout.VERTICAL)
      ) {
      ...
      if (
        ...
        if (item.layout === Form.Layout.HORIZONTAL) {
          ...
          sliderSize = [
            sliderTickSize,
            sliderTickSize * 1.5 * item.options.length,
          ];
          compact = false;
          flip = true;
        }
  ...
  _updateIfNeeded() {
    ...
    this._scrollbar.setPos(
      [this._rightEdge - this._scrollbarWidth / 2, this._pos[1]],
      false,
    );
    this._scrollbar.setOpacity(0.5);
    this._scrollbarOffset =
      (this._prevScrollbarMarkerPos *
        (this._visual.stimuliTotalHeight + this._size[1])) /
      (-this._visual.stimuliTotalHeight / this._size[1]);
  ...
  _updateVisibleStimuli() {
    for (let i = 0; i < this._items.length; ++i) {
      ...
      if (
        textStimPos[1] > this._bottomEdge &&
        textStimPos[1] - this._visual.rowHeights[i] <= this._topEdge
      ) {
