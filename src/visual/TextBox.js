...
export class TextBox extends util.mix(VisualStim).with(ColorMixin) {
  ...
  constructor({
    ...
    this._addAttribute("alignment", alignment, "center");
    this._addAttribute("languageStyle", languageStyle, "LTR");

    // colors:
    this._addAttribute("color", color, undefined);
    this._addAttribute("fillColor", fillColor, undefined);
    this._addAttribute("borderColor", borderColor, undefined);
  ...
  _addEventListeners() {
    this._pixi.on("input", (textContent) => {
      ...
      if (this._fitToContent) {
        ...
        const size = util.to_unit(
          [this._pixi.width, this._pixi.height],
          "pix",
          this._win,
          this._units,
        );
  ...
  _updateIfNeeded() {
    ...
    if (this._needPixiUpdate) {
      ...
      const prevText =
        this._pixi !== undefined && this._pixi.parent !== null
          ? this._pixi.text
          : this._text;
      ...
      const otherTextBoxWithFocus = _drawList.some(
        (item) =>
          item instanceof TextBox && item._pixi && item._pixi._hasFocus(),
      );
    ...
    [this._pixi.x, this._pixi.y] = util.to_px(
      this._pos,
      this._units,
      this._win,
    );
