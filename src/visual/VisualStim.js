...
export class VisualStim extends util.mix(MinimalStim).with(WindowMixin) {
  ...
  constructor({
    ...
    this._addAttribute("pos", pos, [0, 0]);
    this._addAttribute("anchor", anchor, "center");
    this._addAttribute("size", size, undefined);
    this._addAttribute("ori", ori, 0.0);
    this._addAttribute("opacity", opacity, 1.0, this._onChange(true, false));
    this._addAttribute("depth", depth, 0, this._onChange(false, false));
    this._addAttribute(
      "clipMask",
      clipMask,
      null,
