...
export class Camera extends PsychObject {
  constructor({ autoLog, clock, format, name, win } = {}) {
    super(win._psychoJS);
  ...
  _prepareRecording() {
    ...
    this._psychoJS.logger.debug(
      `camera stream settings: ${JSON.stringify(this._streamSettings)}`,
    );

    // check that the specified format is supported, use default if it is not:
    let options;
    if (
      typeof this._format === "string" &&
      MediaRecorder.isTypeSupported(this._format)
    ) {
      options = { type: this._format };
    } else {
      this._psychoJS.logger.warn(
        `The specified video format, ${this._format}, is not supported by this browser, using the default format instead`,
      );
    }
    ...
    this._recorder.onerror = (event) => {
      ...
      self._psychoJS.logger.error(
        "video recording error: " + JSON.stringify(event),
      );
  ...
  async authorize(showDialog = false, dialogMsg = undefined) {
    ...
    if (showDialog) {
      dialogMsg ??=
        "Please wait a few moments while the camera initialises. You may need to grant permission to your browser to use the camera.";
  ...
  open() {
    if (this._stream === null) {
      throw {
        ...
        error:
          "access to the camera has not been authorized, or no camera could be found",
        origin: "Camera.open",
      };
    }
  ...
  pause() {
    if (this._status === PsychoJS.Status.STARTED) {
      ...
      try {
        ...
        self._psychoJS.logger.error(
          "unable to pause the video recording: " + JSON.stringify(error),
        );
        this._status = PsychoJS.Status.ERROR;
    ...
    if (this._status !== PsychoJS.Status.STARTED) {
      ...
      try {
        ...
        this._psychoJS.logger.error(
          "unable to start the video recording: " + JSON.stringify(error),
        );
        this._status = PsychoJS.Status.ERROR;

        throw {
          context:
            "when starting the video recording for camera: " + this._name,
          error,
  ...
  resume({ clear = false } = {}) {
    if (this._status === PsychoJS.Status.PAUSED) {
      ...
      try {
        ...
        self._psychoJS.logger.error(
          "unable to resume the video recording: " + JSON.stringify(error),
        );
        this._status = PsychoJS.Status.ERROR;

        throw {
          context:
            "when resuming the video recording for camera: " + this._name,
          error,
