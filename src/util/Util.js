...
export function to_norm(pos, posUnit, win) {
  ...
  if (posUnit === "height") {
    const minSize = Math.min(win.size[0], win.size[1]);
    return [
      (pos[0] * minSize) / (win.size[0] / 2.0),
      (pos[1] * minSize) / (win.size[1] / 2.0),
    ];
  }
...
export function to_height(pos, posUnit, win) {
  ...
  if (posUnit === "norm") {
    const minSize = Math.min(win.size[0], win.size[1]);
    return [
      (pos[0] * win.size[0]) / 2.0 / minSize,
      (pos[1] * win.size[1]) / 2.0 / minSize,
    ];
  }
...
export function flattenArray(array) {
  return array.reduce((flat, next) => {
    flat.push(
      Array.isArray(next) && Array.isArray(next[0]) ? flattenArray(next) : next,
    );
    return flat;
  }, []);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    download.onload = () => {
      ...
      if (downloadCounter === nbDownloads) {
        const speed_bps =
          (imageSize_b * nbDownloads) / (downloadTimeAccumulator / 1000);
