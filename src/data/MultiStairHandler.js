...
export class MultiStairHandler extends TrialHandler {
  ...
  constructor({
    ...
    this._addAttribute(
      "stairType",
      stairType,
      MultiStairHandler.StaircaseType.SIMPLE,
    );
  ...
  _nextTrial() {
    this._psychoJS.logger.debug(
      `current staircase (before update)= ${this._currentStaircase}`,
    );

    try {
      // if the current pass is empty, get a new one:
      if (this._currentPass.length === 0) {
        this._currentPass = this._staircases.filter(
        ...
        if (this._multiMethod === TrialHandler.Method.SEQUENTIAL) {
          ...
          this._currentPass = util.shuffle(
            this._currentPass,
          ...
          if (this._currentPass.length > 0) {
            ...
            const index = Math.floor(
              this._randomNumberGenerator() * this._currentPass.length,
            );
      ...
      this._psychoJS.logger.debug(
        `selected staircase: ${this._currentStaircase.name}, estimated value for variable ${this._varName}: ${value}`,
      );
      ...
      for (let t = 0; t < this._trialList.length; ++t) {
        if (typeof this._trialList[t] === "undefined") {
          ...
          for (const attribute of this._currentStaircase._userAttributes) {
            ...
            if (attribute === "name") {
              this._trialList[t][this._name + ".label"] =
                this._currentStaircase["_name"];
            } else if (attribute !== "trialList" && attribute !== "extraInfo") {
              this._trialList[t][this._name + "." + attribute] =
                this._currentStaircase["_" + attribute];
            }
          }

          console.log("@@@@", this._currentStaircase._extraArgs);
          for (const arg in this._currentStaircase._extraArgs) {
            this._trialList[t][this._name + "." + arg] =
              this._currentStaircase._extraArgs[arg];
            this._trialList[t][arg] = this._currentStaircase._extraArgs[arg];
          }

          this._psychoJS.logger.debug(
            `updated the trialList at index: ${t} to: ${JSON.stringify(this._trialList[t])}`,
          );
