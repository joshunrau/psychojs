...
export class StairHandler extends TrialHandler {
  ...
  _estimateStairValue() {
    this._psychoJS.logger.debug(
      `stairValue before update= ${this._stairValue}, currentDirection= ${this._currentDirection.toString()}, correctCounter= ${this._correctCounter}`,
    );
    ...
    if (reverseDirection) {
      ...
      this._initialRule =
        this.reversalIntensities.length === 0 && this._applyInitialRule;
      this.reversalIntensities.push(this._values.at(-1));
    }

    // check whether we should finish the trial:
    if (
      this.reversalIntensities.length >= this._nReversals &&
      this._values.length >= this._nTrials
    ) {
      this._finished = true;
    ...
    if (reverseDirection && this._variableStep) {
      ...
      if (this.reversalIntensities.length >= this._stepSizes.length) {
        ...
        this._currentStepSize = this._stepSizes.at(
          this.reversalIntensities.length,
        );
      }
    }
    ...
    if (
      (this.reversalIntensities.length === 0 || this._initialRule) &&
      this._applyInitialRule
    ) {
      this._initialRule = false;
    ...
    this._psychoJS.logger.debug(
      `estimated value for variable ${this._varName}: ${this._stairValue}`,
    );
