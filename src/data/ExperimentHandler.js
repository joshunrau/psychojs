...
export class ExperimentHandler extends PsychObject {
  ...
  constructor({ psychoJS, name, extraInfo, dataFileName } = {}) {
    ...
    this._experimentName =
      typeof extraInfo.expName === "string" && extraInfo.expName.length > 0
        ? extraInfo.expName
        : this.psychoJS.config.experiment.name;
    this._participant =
      typeof extraInfo.participant === "string" &&
      extraInfo.participant.length > 0
        ? extraInfo.participant
        : "PARTICIPANT";
    this._session =
      typeof extraInfo.session === "string" && extraInfo.session.length > 0
        ? extraInfo.session
        : "SESSION";
    this._datetime =
      typeof extraInfo.date !== "undefined"
        ? extraInfo.date
...
*/
      ...
      let csv =
        (withHeader ? "\ufeff" : "") + XLSX.utils.sheet_to_csv(worksheet);
      if (data.length > 0) {
        csv += "\n";
      }
