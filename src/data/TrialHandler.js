    ...
    try {
      ...
      if (["csv", "odp", "xls", "xlsx"].indexOf(resourceExtension) > -1) {
        ...
        const decodedResource =
          type === "string"
            ? new TextDecoder().decode(decodedResourceMaybe)
            : decodedResourceMaybe;
  ...
  _prepareSequence() {
    ...
    if (this._method === TrialHandler.Method.SEQUENTIAL) {
      ...
      for (let i = 0; i < this.nReps; ++i) {
        this._trialSequence.push(
          util.shuffle(indices.slice(), this._randomNumberGenerator),
        );
      }
      ...
      for (let i = 0; i < this.nReps; i++) {
        this._trialSequence.push(
          flatSequence.slice(i * this.nStim, (i + 1) * this.nStim),
        );
      }
    } else {
